/**
 * Workflow-графы студии OS — те же пресеты, что frontend/src/studio/studioScenarioPresets.ts
 * и шаблоны backend/data/workflow_templates (po-refu, smena-modeli, smena-lokacii, smena-odezhdy).
 */
;(function (window) {
  'use strict'
  ...
  /** Как smena-odezhdy.json — refs напрямую в imageGeneration (без scenarioOutfitChange). */
...
async function resolveRefId(API, store, archiveThumbUrl, source) {
    const preferArchive = source && source.preferredSource === 'archive'
  ...
  async function buildGraphForMode(mode, ctx) {
    ...
    if (mode === 'edit') {
      ...
      if (s?.needsRef === 'yes') {
        ...
        detailRefId = await resolveRefId(API, store, archiveThumbUrl, slot1)
      }
