...
export function CabinetDataProvider({ children }) {
  ...
  const sendVideoNoteReply = useCallback(
    async (convId, { text = '', renderId = null, generationId = null, replyToMessageId = null } = {}) => {
      if (!convId || (!renderId && !generationId)) return
      setError(null)
      try {
        const sent = await actions.sendVideoNoteReply(convId, {
          text,
          renderId,
          generationId,
          replyToMessageId,
  ...
  const uploadMotionControlFirstFrame = useCallback(
    (params) => actions.uploadMotionControlFirstFrame(params),
    [],
  )
