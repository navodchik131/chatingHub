...
;(function (global) {
  ...
  function adminPanelUrl() {
    try {
      ...
      if (port === '5180') {
        return window.location.protocol + '//' + window.location.hostname + ':8080/admin'
      }
    } catch (_) {}
    return '/admin'
  }
  ...
  const PHOTO_TAG_DEFS = [
    { kind: 'face', ru: 'Лицо / идентичность', en: 'Face / identity' },
    { kind: 'turnaround', ru: 'Развёртка', en: 'Turnaround / character sheet' },
    { kind: 'body', ru: 'Тело целиком', en: 'Full body' },
    { kind: 'genitals', ru: 'Интимная зона (реф.)', en: 'Intimate reference' },
    { kind: 'other', ru: 'Общий референс', en: 'General reference' },
  ]
  ...
  function mapPhotoTagList(lang, logic) {
    ...
    const defs = [
      { kind: 'face', label: lang === 'ru' ? 'Лицо / идентичность' : 'Face / identity' },
      { kind: 'turnaround', label: lang === 'ru' ? 'Развёртка' : 'Turnaround / character sheet' },
      { kind: 'body', label: lang === 'ru' ? 'Тело целиком' : 'Full body' },
      { kind: 'genitals', label: lang === 'ru' ? 'Интимная зона (реф.)' : 'Intimate reference' },
      { kind: 'other', label: lang === 'ru' ? 'Общий референс' : 'General reference' },
    ]
    return defs.map((d) => ({
      kind: d.kind,
      label: d.label,
