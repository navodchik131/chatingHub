...
def is_channel_dm_message(message: Message) -> bool:
    ...
    if message.message_thread_id is not None:
        return True
    # Some channel DM media updates omit thread/topic — still route if chat is DM inbox.
...
def resolve_channel_dm_topic_id(message: Message) -> str | None:
    ...
    if getattr(message.chat, "is_direct_messages", None) is True and from_user is not None:
