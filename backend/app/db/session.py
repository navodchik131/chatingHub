...
async def init_db() -> None:
    ...
    async with engine.begin() as conn:
        ...
        await conn.run_sync(_migrate_telegram_mobile_auth_partner_attribution)
...
def _migrate_connection_companion_columns(sync_conn) -> None:
    ...
    for table in _CONNECTION_TABLES:
        ...
        def add_col(sql_if_not_exists: str, sql_plain: str) -> None:
            if dialect == "postgresql":
                sync_conn.execute(text(sql_if_not_exists))
            else:
        ...
        if "companion_goal_preset" not in cols:
            add_col(
                f"ALTER TABLE {table} ADD COLUMN IF NOT EXISTS "
                f"companion_goal_preset VARCHAR(16) DEFAULT 'chat'",
                f"ALTER TABLE {table} ADD COLUMN companion_goal_preset "
                f"VARCHAR(16) DEFAULT 'chat'",
            )
        if "companion_goal_text" not in cols:
            add_col(
                f"ALTER TABLE {table} ADD COLUMN IF NOT EXISTS companion_goal_text TEXT",
                f"ALTER TABLE {table} ADD COLUMN companion_goal_text TEXT",
            )
        if "companion_goal_link" not in cols:
            add_col(
                f"ALTER TABLE {table} ADD COLUMN IF NOT EXISTS "
                f"companion_goal_link VARCHAR(512)",
...
def _migrate_telegram_mobile_auth_sessions(sync_conn) -> None:
    ...
    insp = inspect(sync_conn)
    if insp.has_table("telegram_mobile_auth_sessions"):
        return
    sync_conn.exec_driver_sql(
        """
        CREATE TABLE telegram_mobile_auth_sessions (
    ...
    sync_conn.exec_driver_sql(
        "CREATE INDEX IF NOT EXISTS ix_telegram_mobile_auth_sessions_status "
        "ON telegram_mobile_auth_sessions(status)"
    )
...
def _migrate_exif_bot_telegram_id_bigint(sync_conn) -> None:
    import logging
    ...
    if not insp.has_table("exif_bot_users"):
        return
    for col in insp.get_columns("exif_bot_users"):
        if col["name"] != "telegram_id":
            continue
        type_str = str(col.get("type", "")).upper()
        if "BIGINT" in type_str or "INT8" in type_str:
    ...
    if dialect == "postgresql":
        sync_conn.execute(
            text(
                "ALTER TABLE exif_bot_users "
                "ALTER COLUMN telegram_id TYPE BIGINT USING telegram_id::bigint"
            )
        )
        log.info("exif_bot_users: telegram_id migrated to BIGINT")
...
def _migrate_telegram_mobile_auth_partner_attribution(sync_conn) -> None:
    from sqlalchemy import inspect, text
    ...
    if not insp.has_table("telegram_mobile_auth_sessions"):
        return
    cols = {c["name"] for c in insp.get_columns("telegram_mobile_auth_sessions")}
    if "partner_slug" not in cols:
        sync_conn.execute(
            text("ALTER TABLE telegram_mobile_auth_sessions ADD COLUMN partner_slug VARCHAR(32)")
        )
    if "partner_source_tag" not in cols:
        sync_conn.execute(
            text(
                "ALTER TABLE telegram_mobile_auth_sessions "
                "ADD COLUMN partner_source_tag VARCHAR(64)"
            )
        )
