...
def _resolve_pack_media(
    *,
    ...
    return PaidPackMedia(relative_paths=paths, media_type=media_type, asset_ids=ids)
...
async def send_unibox_paid_pack(
    ...
    *,
    ...
    """
    Создаёт заказ, шлёт send_paid_media, пишет исходящее в тред.
    Возвращает (message_id, order_id).
    """
    ...
    if stars < 1:
        raise HTTPException(
            status_code=400,
            detail="Укажите цену ⭐ для пака в медиатеке персонажа",
        )
    if conv.studio_model_id and int(conv.studio_model_id) != int(pack.studio_model_id):
        raise HTTPException(
            status_code=400,
            detail="Пак относится к другому персонажу",
        )
    ...
    label = f"⭐ Платный контент · {pack.name} · {stars} ⭐"
    if caption and caption.strip():
        label = f"{caption.strip()}\n\n{label}"

    meta = merge_meta_dict(
        None,
        {
            "stars_paid_media": {
                "order_id": order.id,
    ...
    await session.flush()
    log.info(
        "unibox paid media sent conv=%s pack=%s order=%s tg_msg=%s",
        conv.id,
        pack.id,
        order.id,
        result.message_id,
