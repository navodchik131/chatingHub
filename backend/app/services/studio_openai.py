...
def _wavespeed_pose_ref_prefix(*, lock_model_hairstyle: bool) -> str:
    ...
    return (
        ...
        "**Do not copy donor bust/waist/hip mass, limb thickness, or torso length from image 1** — only joint angles, "
        "camera, framing, light wrap, and coverage zones. "
        "**Do not do a face-swap:** synthesize **one cohesive person** in this scene. "
        "**Volumetric silhouette** (bust, waist, hips, shoulders, glute volume) and **all visible skin** "
        "(face, neck, chest, arms, torso, legs) must match the identity reference images (following URLs) — "
        "same MODEL tone, same grain, **same light falloff** on face and body (reference **direction** from image 1, "
        "not donor complexion or body mass). "
...
def _wavespeed_face_swap_prefix_scene_first(*, lock_model_hairstyle: bool) -> str:
    ...
    return (
        ...
        "**Replace** every visible face and epidermis region in Image 1 with MODEL identity from Images 2–3 continuously — "
...
def _wavespeed_face_swap_prefix_face_hidden(*, lock_model_hairstyle: bool, scene_first: bool = False) -> str:
    if scene_first:
        hair_clause = (
            "**Visible hair mass** follows Image 1 pose crop; color/texture from Image 2 MODEL only. "
            if lock_model_hairstyle
            else "Hair visible in Image 1 may keep that layout; color from Image 2 when applicable. "
        )
        return (
            f"{_FACE_SWAP_TAG} **Image 1** = **SOURCE snapshot** (pose/framing canvas). "
            "**Image 2** = **MODEL face** (partial identity anchor). "
            "**Image 3** = **MODEL body** proportions reference. "
            "**Replace** visible epidermis in Image 1 with MODEL identity — not the incidental sitter. "
            f"{hair_clause}"
            "Image 1 defines pose + illumination; Images 2–3 supply who.\n\n"
        )
    hair_clause = (
    ...
    return (
        f"{_FACE_SWAP_TAG} **Image 1** = **MODEL face** (identity anchor when partially visible). "
...
async def generate_model_profile_json_from_images(
    *,
    ...
    async def _vision_call(*, model: str, creds: StudioOpenAiCredentials | None) -> str:
        return await chat_completion_openai_compatible_text(
            model=model,
            messages=messages,
            max_tokens=8192,
            temperature=0.35,
