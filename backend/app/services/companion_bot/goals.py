...
def _preset_strategy(preset: str) -> str:
    if preset == "funnel":
        return (
            "Soft traffic funnel: keep the chat warm and human; when the fan is engaged, "
            "occasionally hint that more content lives elsewhere (Telegram channel, private group, "
            "or link) — curiosity, not a billboard."
        )
    if preset == "sales":
        return (
